...
function sortedInventoryList(){
 ...
 let list=vehicles.filter(v=>(!q||`${v.title} ${v.make} ${v.model} ${v.year} ${v.vin}`.toLowerCase().includes(q))&&(!status||v.status===status));
